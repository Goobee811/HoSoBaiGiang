...
SRC = os.path.join(REPO, 'drafts_v2')
OUT = os.path.join(REPO, 'Ho_so_bai_giang_lai_xe_oto_2026_v2.docx')
...
# ───────── inline markdown ─────────
def runs(par, text, bold=False, italic=False, size=BODY):
    text = re.sub(r'<br\s*/?>', '\n', text).replace('\\*', '\x00')
    b, it = bold, italic
    buf = []
    def flush():
        ...
        for n, piece in enumerate(s.split('\n')):
            ...
            if piece:
                setfont(par.add_run(piece), size, b, it)
    i, n = 0, len(text)
    while i < n:
...
for f in FILES:
    ...
    while i < len(lines):
        ...
        if s.startswith('# '):
            ...
            i += 1; continue
        if s.startswith('### '):
            p = para(WD_ALIGN_PARAGRAPH.LEFT, before=8, keep=True)
            runs(p, s[4:].strip(), bold=True)
            i += 1; continue
        if s.startswith('## '):
            p = para(WD_ALIGN_PARAGRAPH.LEFT, before=10, keep=True)
