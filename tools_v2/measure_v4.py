...
DOCX = os.path.join(REPO, 'Ho_so_bai_giang_lai_xe_oto_2026_v4.docx')
PDF  = os.path.join(WORK, 'pdf', 'Ho_so_bai_giang_lai_xe_oto_2026_v4.pdf')

d = docx.Document(DOCX)
heads = []
for p in d.paragraphs:
    sn, t = p.style.name, p.text.strip()
    if not t or t == 'MỤC LỤC':
        continue
    if sn == 'Heading 1':
        heads.append((1, t))
    elif sn == 'Heading 2':
...
for i, pg in enumerate(pages[:10]):
    if 'MỤC LỤC' in pg:
        toc_pages.add(i)
        j = i + 1
        while j < len(pages) and 'CHƯƠNG TRÌNH MÔN HỌC' not in pages[j]:
            toc_pages.add(j); j += 1
        break
...
json.dump(out, open(os.path.join(WORK, 'toc_v4.json'), 'w', encoding='utf8'), ensure_ascii=False, indent=0)
